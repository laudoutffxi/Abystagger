...
-- Vana-diel time: use the canonical Ashita vanatime pointer chain.
-- The signature scan resolves the pointer at +0x34; that pointer then points
-- to the clock structure, whose raw timestamp is at +0x0C.
--
-- Canonical Ashita conversion:
--   ts = (raw_timestamp + 92514960) * 25
--   3600 Vana seconds = 1 Vana hour
--   86400 Vana seconds = 1 Vana day
local VANA_EPOCH_OFFSET = 92514960

local function init_vana_time()
    -- This is the same signature/offset used by Ashita's ffxi.vanatime lib.
...
local function update_vana_time()
    ...
    -- pVanaTime is already the resolved pointer address (findpattern + 0x34).
    local clock_ptr = ashita.memory.read_uint32(state.pVanaTime)
    if clock_ptr == nil or clock_ptr == 0 then
        return
    end

    -- Raw timestamp is stored at +0x0C in the clock structure.
...
local function update_target()
    ...
    if target ~= nil and target.Name ~= nil and target.Name ~= '' then
...
local function blue_window()
    -- Blue uses the exact same live Vana hour shown in the header.
...
local function render_header()
    imgui.TextColored(colors.yellow, 'FFXI ABYSSEA STAGGER')
    imgui.SameLine()
    imgui.Text('  -  LIVE')
    imgui.Separator()

    imgui.TextColored(colors.white, 'Target:')
    imgui.SameLine()
    imgui.Text(state.target)

    imgui.SameLine()
    imgui.TextColored(colors.gray, ' | ')

    imgui.SameLine()
    imgui.TextColored(colors.yellow, ("Vana-diel %02d:%02d"):format(state.vana_hour, state.vana_minute))

    imgui.SameLine()
    imgui.TextColored(colors.yellow, ' ' .. current_day())
    imgui.Separator()
end

local function render_red()
    imgui.TextColored(colors.red, '!! RED !!')
    imgui.SameLine()
    imgui.Text('Elemental Weapon Skills  -  Random WS trigger')

    imgui.BeginChild('red_list_v2', {0, 185}, ImGuiChildFlags_Borders)
...
local function render_blue()
    ...
    imgui.TextColored(colors.blue, '!! BLUE !!')
    imgui.SameLine()
    imgui.Text(('Physical Weapon Skills  -  ACTIVE: %s'):format(active))

    imgui.BeginChild('blue_list_v2', {0, 260}, ImGuiChildFlags_Borders)
...
local function render_yellow()
    ...
    local active_element = YELLOW[state.vana_weekday + 1]
...
ashita.events.register('command', 'command_cb', function (e)
    ...
    if args[1] ~= '/abystagger' and args[1] ~= '/as' then
        return
    end
...
ashita.events.register('d3d_present', 'present_cb', function ()
    ...
    imgui.SetNextWindowSize({ 760, 760 }, ImGuiCond_FirstUseEver)
    imgui.SetNextWindowSizeConstraints({ 600, 500 }, { 1200, 1200 })

    if imgui.Begin('Abyssea Stagger##abystagger_v2', state.open) then
        render_header()

        if imgui.Button(state.show_all[1] and 'CURRENT ONLY' or 'SHOW ALL') then
            state.show_all[1] = not state.show_all[1]
        end

        imgui.SameLine()
        imgui.TextColored(colors.gray, '/abystagger  toggle')
        imgui.SameLine()
        imgui.TextColored(colors.gray, '| /abystagger all  toggle all')
        imgui.SameLine()
        imgui.TextColored(colors.gray, '| /abystagger compact')
        ...
        imgui.Separator()
        imgui.TextColored(colors.gray,
            'Red: random elemental WS  |  Blue: time-of-day  |  Yellow: day-of-week')
        imgui.TextColored(colors.gray,
            'Blue windows: 06:00-13:59 Piercing - 14:00-21:59 Slashing - 22:00-05:59 Blunt')
    end
